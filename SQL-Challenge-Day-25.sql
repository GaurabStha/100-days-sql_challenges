-- American Express (Medium Level) #SQL Interview Question

-- American Express is reviewing their customers' transactions, 
-- and you have been tasked with locating the customer who has the 
-- third highest total transaction amount. The output should include 
-- the customer's id, as well as their first name and last name. For 
-- ranking the customers, use type of ranking with no gaps between subsequent ranks.

create database AmericanExpressDB;
use AmericanExpressDB;
...
with customer_transaction as (
	select
		cust_id,
		sum(total_order_cost) as total_transaction
	from
		card_orders
	group by
		cust_id
),
cust_transaction_rank as (
	select *,
		dense_rank() over (order by total_transaction desc) as cust_rank
	from 
