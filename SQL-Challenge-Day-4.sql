...
-- First calculated the total monthly distance to travel and monetary cost group by month
with monthly_stats as (
    select 
        date_format(request_date, '%Y-%m') as request_year_month,
        sum(distance_to_travel) AS total_distance_to_travel,
        sum(monetary_cost) AS total_monetary_cost
    from uber_request_logs
    group by request_year_month
),
-- calculated distance per dollar
distance_per_dollar as (
    select
        request_year_month,
        total_distance_to_travel,
        total_monetary_cost,
        (total_distance_to_travel / total_monetary_cost) AS distance_per_dollar
    from monthly_stats
),
-- made the previous month values as forecasted values
monthly_with_forecasted AS (
	select
        request_year_month,
        total_distance_to_travel,
        total_monetary_cost,
        distance_per_dollar,
        lag(distance_per_dollar) over (order by request_year_month) as forecasted_value
    from distance_per_dollar
)
-- calculated RMSE
select
    round(sqrt(avg(power(distance_per_dollar - forecasted_value, 2))), 2) as rmse
from monthly_with_forecasted
